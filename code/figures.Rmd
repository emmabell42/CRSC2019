...
```{r}
cost <- read.csv(file = file.path(data.dir,"raw_data/Sequencing_Costs_Table_July_2017.csv"), header = TRUE, stringsAsFactors = FALSE)
cost <- cost[,1:4]
...
#How many samples are in GEO?

```{r}
geo <- read.csv(file = file.path(data.dir,"raw_data/20190523_GEOcontent.csv"), header = TRUE, stringsAsFactors = FALSE)
...
```{r}
dummy.data <- array(data = NA, dim = c(1000,6))
colnames(dummy.data) <- sort(paste(c("Case","Control"),rep(1:3, 2), sep = "_"))
#dummy.data[,c(1,2,4)] <- 
length(lapply(X = dummy.data[,c(1,2,4)], FUN = function(X) rnorm(n = 1000, mean = 50, sd = 10)))
dummy.data[,c(3,5,6)] <- lapply(X = dummy.data[,c(3,5,6)], FUN = function(X) rnorm(n = 1000, mean = 150, sd = 20))
dim(dummy.data)
```
